...
ruleStotinka = Rule
  ...
  , prod = \_ -> Just . Token AmountOfMoney $ currencyOnly Stotinka
  }
...
ruleIntersectAndXStotinki :: Rule
ruleIntersectAndXStotinki = Rule
  { name = "intersect (and X stotinki)"
  , pattern =
    [ financeWith TAmountOfMoney.value isJust
    , regex "и"
    , financeWith TAmountOfMoney.currency (== Stotinka)
    ]
  , prod = \tokens -> case tokens of
      (Token AmountOfMoney fd:
       _:
       Token AmountOfMoney (AmountOfMoneyData {TAmountOfMoney.value = Just c}):
       _) -> Just . Token AmountOfMoney $ withStotinki c fd
      _ -> Nothing
  }
