...
--     [ regex "თეთრი?ს?(ად)?თ?"
--     ]
--   , prod = \_ -> Just . Token AmountOfMoney $ currencyOnly TETRI
--   }
...
--   { name = "cent"
--   , pattern =
--     [ regex "ევრო ?ცენტი?ს?(ად)?თ?"
--     ]
--   , prod = \_ -> Just . Token AmountOfMoney $ currencyOnly EuroCent
--   }
...
-- ruleIntersectAndXTetris :: Rule
-- ruleIntersectAndXTetris = Rule
--   { name = "intersect (and X tetris)"
--   , pattern =
--     [ Predicate isWithoutTetris
--     , regex "და"
--     , Predicate isTetris
--     ]
--   , prod = \tokens -> case tokens of
...
--        _:
--        Token AmountOfMoney AmountOfMoneyData{TAmountOfMoney.value = Just c}:
--        _) -> Just . Token AmountOfMoney $ withTetris c fd
--       _ -> Nothing
--   }
