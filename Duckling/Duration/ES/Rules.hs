...
ruleDurationQuarterOfAnHour = Rule
  ...
  , pattern =
    [ regex "(1/4\\s?h(ora)?|(un\\s)?cuarto de hora)"
    ]
...
ruleDurationThreeQuartersOfAnHour = Rule
  ...
  , pattern =
    [ regex "(3/4\\s?h(ora)?|tres(\\s|-)cuartos de hora)"
    ]
...
ruleDurationNumeralMore = Rule
  { name = "<integer> <unit-of-duration> more"
  , pattern =
    [ Predicate isNatural
    , dimension TimeGrain
    , regex "m(á|a)s|adicional|extra|menos"
    ]
  , prod = \case
...
ruleDurationPrecision = Rule
  ...
  , pattern =
    [ regex "(en|alrededor|aproximadamente|exactemente)"
    , dimension Duration
    ]
    , prod = \case
