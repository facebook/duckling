...
ruleTens = Rule
  { name = "integer (20..90)"
  , pattern = [regex "((два|три|четири|пет|шест|седем|осем|девет)десет)"]
  , prod = \tokens ->
      case tokens of
        (Token RegexMatch (GroupMatch (match:_)):_) ->
          case Text.toLower match of
            "двадесет"  -> integer 20
            "тридесет"  -> integer 30
            "четиридесет"   -> integer 40
            "петдесет"   -> integer 50
            "шестдесет"   -> integer 60
            "седемдесет" -> integer 70
            "осемдесет"  -> integer 80
            "деведесет"  -> integer 90
            _         -> Nothing
        _ -> Nothing
  }
...
ruleCompositeTens = Rule
  { name = "integer 21..99"
  , pattern = [oneOf [20,30..90], regex "и", numberBetween 1 10]
  , prod = \tokens -> case tokens of
...
ruleHundreds = Rule
  ...
  , pattern = [regex "(сто|двеста|триста|(четири|пет|шест|седем|осем|девет)стотин)"]
  , prod = \tokens ->
      case tokens of
        (Token RegexMatch (GroupMatch (match:_)):_) ->
          case Text.toLower match of
...
ruleCompositeHundreds = Rule
  ...
  , pattern = [oneOf [200,300..900], numberBetween 1 100]
  , prod = \tokens -> case tokens of
