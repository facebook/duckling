...
ruleConstitutionDay = Rule
  ...
  , prod = \_ -> Just . Token Time $ monthDay 6 5
  }
...
ruleOrdinalQuarter = Rule
  ...
  , prod = \tokens -> case tokens of
      (token:_) -> do
        n <- getIntValue token
        Just . Token Time . cycleNthAfter False TG.Quarter (n - 1) $
          cycleNth TG.Year 0
      _ -> Nothing
  }
  ...
  , prod = \tokens -> case tokens of
      (token:
       ...
       _) -> do
         ...
         Just . Token Time $ intersect (td2, year n)
      _ -> Nothing
  }
...
ruleHhmmss = Rule
  ...
  , prod = \tokens -> case tokens of
      (Token RegexMatch (GroupMatch (m1:m2:m3:_)):_) -> do
        ...
        m <- parseInt m2
        s <- parseInt m3
        Just . Token Time $ hourMinuteSecond False h m s
      _ -> Nothing
  }
