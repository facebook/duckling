...
ruleTimeofdayLatent = Rule
  ...
  , prod = \tokens -> case tokens of
      (token:_) -> do
        n <- getIntValue token
        Just . Token Time . mkLatent $ hour (n < 12) n
      _ -> Nothing
  }
...
ruleLastNCycle = Rule
  ...
  , prod = \tokens -> case tokens of
      (_:token:Token TimeGrain grain:_) -> do
        n <- getIntValue token
        Just . Token Time $ cycleN True grain (- n)
      _ -> Nothing
  }
...
ruleSterreichischerNationalfeiertag = Rule
  ...
  , prod = \_ -> Just . Token Time $ monthDay 10 26
  }
...
ruleNikolaus = Rule
  ...
  , prod = \_ -> Just . Token Time $ monthDay 12 6
  }
...
ruleYearLatent = Rule
  ...
  , prod = \tokens -> case tokens of
      (token:_) -> do
        y <- getIntValue token
        Just . Token Time . mkLatent $ year y
      _ -> Nothing
  }
...
ruleHhmmMilitary = Rule
  ...
  , prod = \tokens -> case tokens of
      (Token RegexMatch (GroupMatch (h:m:_)):_) -> do
        hh <- parseInt h
        mm <- parseInt m
        Just . Token Time . mkLatent $ hourMinute False hh mm
      _ -> Nothing
  }
...
ruleOrdinalQuarter = Rule
  ...
  , prod = \tokens -> case tokens of
      (Token Ordinal (OrdinalData {TOrdinal.value = v}):_) -> Just . Token Time .
  ...
  , prod = \tokens -> case tokens of
      (token:
       ...
       _) -> do
        ...
        Just . Token Time $ intersect (dom, year n)
      _ -> Nothing
  }
