...
ruleHourofdayQuarter = Rule
  ...
  , prod = \tokens -> case tokens of
      (Token Time TimeData {TTime.form = Just (TTime.TimeOfDay (Just hours) _)}:_) ->
        Just . Token Time $ hourMinute True hours 15
      _ -> Nothing
  }
...
ruleHourofdayHalf = Rule
  ...
  , prod = \tokens -> case tokens of
      (Token Time TimeData {TTime.form = Just (TTime.TimeOfDay (Just hours) _)}:_) ->
        Just . Token Time $ hourMinute True hours 30
      _ -> Nothing
  }
...
ruleConstitutionDay = Rule
  ...
  , prod = \_ -> Just . Token Time $ monthDay 5 17
  }
...
ruleFathersDay = Rule
  ...
  , prod = \_ -> Just . Token Time $ nthDOWOfMonth 1 7 11
  }
...
ruleMothersDay = Rule
  ...
  , prod = \_ -> Just . Token Time $ nthDOWOfMonth 1 7 2
  }
  ...
  , prod = \tokens -> case tokens of
      (token:
       ...
       _) -> do
         ...
         Just . Token Time $ intersect (dom, year y)
      _ -> Nothing
  }
